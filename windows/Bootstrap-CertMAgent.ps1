...
$needsBootstrapCredential = $Force -or -not $configurationExists
$needsInitialEnrollment = $needsBootstrapCredential
if ($configurationExists -and -not $Force) {
    $existingConfiguration = Get-Content `
        -LiteralPath $configPath `
        -Raw `
        -Encoding UTF8 | ConvertFrom-Json
    $needsInitialEnrollment = (
        $existingConfiguration.PSObject.Properties.Name -contains
