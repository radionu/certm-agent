...
systemctl disable --now certm-agent-update.timer >/dev/null 2>&1 || true
rm -f \
  /etc/systemd/system/certm-agent-update.service \
  /etc/systemd/system/certm-agent-update.timer
...
echo
echo "CertM Agent 1.0.0-rc.17 installed. Running full preflight before enrollment."
